...
Since the `level0` binary has the setuid bit set, it runs with the privileges of the `level1` user when executed. When we provided the correct argument (423), the program executed `/bin/sh` (a shell) with `level1` privileges using the `execv` function. This effectively gives us a shell as the `level1` user.
