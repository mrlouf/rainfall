...
The `main` function is similar, except that it calls a function `p`. Let's look at that function:
...
This excludes the possibility of using a `RET2LIBC` attack. But this does not mean that we cannot exploit the binary or the `gets` call, we'll just have to use a different approach.
...
In simpler terms, the PLT is like a table of contents that keeps every function call saved under an alias, while the GOT acts as a relay station that points to the actual location of these functions in memory.

If we can overwrite an entry in the GOT, we can redirect function calls to any address we want: if you know about MITM attacks, this is a very similar approach in the sense that we are trying to 'poison' the GOT to redirect function calls, just like a MITM attack poisons the ARP table to redirect network traffic to a different destination.

##  Exploiting the binary

Here the level gives us subtle hints as to out to proceed. The presence of the `gets` function indicates that we can overflow a buffer, but the absence of `system` suggests we need to call the shell ourselves. The check for the high bits of the input address (the `and` and `cmp` instructions) indicates that we cannot return to the stack or the libc, since the ranges `0xb7xxxxxx` and `0xbfxxxxxx` are filtered out. The presence of `strdup` is a clear hint that we should use the heap to store our shellcode, aka the CPU instructions that will spawn our shell.
